...
class ProductMaterialModel extends \common\models\c2\entity\ProductModel
{
            ...
            if (!empty($this->items)) {
                foreach ($this->items as $item) {
                    ...
                    if (isset($item['id']) && $item['id'] == '') {  // create new items
                        ...
                        $itemStock = new ProductStock();
                        $attrs = [
                            'warehouse_id' => $this->warehouse_id,
                            'product_id' => $this->id,
                            'product_material_id' => $itemModel->id,
                            'num' => 0,
                        ];
                        $itemStock->setAttributes($attrs);
                        $itemStock->save();
    ...
    {
        return $this->hasMany(ProductMaterialItemModel::className(), ['product_id' => 'id']);
    }

    public function getMaterialItemOptions($key = 'id', $val = 'value', $params = []) {
        $options = ArrayHelper::map($this->getProductMaterialItems()->asArray()->all(), $key, $val);
        return $options;
    }
