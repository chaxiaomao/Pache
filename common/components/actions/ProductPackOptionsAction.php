 ...
 * @license
 */
class ProductPackOptionsAction extends \yii\base\Action {
    ...
    protected function prepareDataProvider($params = []) {
        ...
        $model = \backend\models\c2\entity\ProductModel::findOne(['id' => $params['depdrop_all_params']['product_id']]);
        ...
        $options = [];
        foreach ($model->getPackItems()->all() as $item) {
            $options[] = [
                'id' => $item->id,
                'label' =>  $item->label,
            ];
