...
class OrderModel extends \cza\base\models\ActiveRecord
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if (!empty($this->items)) {
            foreach ($this->items as $item) {
                $package_id  = isset($item['product_package_id']) ? $item['product_package_id'] : 0;
                $number = isset($item['number']) ? $item['number'] : 0;
                $pieces = isset($item['pieces']) ? $item['pieces'] : 0;
                if (isset($item['number']) && $item['number'] == '') {
                    $packageModel = ProductPackageModel::findOne($package_id);
                    if (!is_null($packageModel)) {
                        $number = $packageModel->number * $pieces;
                    }
                }
                $attributes = [
    ...
    public function setStateToUntrack()
    {
        ...
        foreach ($this->orderItems as $orderItem) {
            ...
            $combinationModel = $orderItem->productCombination;
            if (!is_null($combinationModel)) {
                foreach ($combinationModel->productCombinationItems as $productCombinationItem) {
                    $attributes = [
                        'order_id' => $this->id,
                        ...
                        'subtotal' => $productCombinationItem->number * $orderItem->number,
                        'memo' => "",
                    ];
            ...
            $packageModel = $orderItem->productPackage;
            if (!is_null($packageModel)) {
                foreach ($packageModel->productPackageItems as $productPackageItem) {
                    $attributes = [
                        'order_id' => $this->id,
                        'order_item_id' => $orderItem->id,
                        'product_id' => $productPackageItem->product_id,
                        'product_number' => $orderItem->pieces,
                        'consumed_number' => $productPackageItem->number,
                        'subtotal' => $productPackageItem->number * $orderItem->pieces,
                        'memo' => "",
                    ];
                    $consumptionModel = new OrderItemConsumptionModel();
                    $consumptionModel->setAttributes($attributes);
